...
import com.example.vespatacho.mlkit.BitmapUtils.getBitmap
import com.example.vespatacho.mlkit.CameraImageGraphic
import com.example.vespatacho.mlkit.FrameMetadata
import com.example.vespatacho.mlkit.GraphicOverlay
import com.example.vespatacho.mlkit.InferenceInfoGraphic
import com.example.vespatacho.mlkit.PreferenceUtils.isCameraLiveViewportEnabled
import com.example.vespatacho.mlkit.PreferenceUtils.shouldHideDetectionInfo
import com.example.vespatacho.mlkit.ScopedExecutor
import com.example.vespatacho.mlkit.VisionImageProcessor
...
abstract class VisionProcessorBase<T> protected constructor(context: Context) : VisionImageProcessor {
    ...
    private fun processImage(
        ...
        val bitmap =
            if (isCameraLiveViewportEnabled(graphicOverlay.getContext()))
                null
            else
                getBitmap(data, frameMetadata)
        ...
        if (isMlImageEnabled(graphicOverlay.getContext())) {
            ...
            requestDetectInImage(mlImage, graphicOverlay, bitmap,  /* shouldShowFps= */true, frameStartMs)
                .addOnSuccessListener(executor, OnSuccessListener { results: T? -> processLatestImage(graphicOverlay) })
    ...
    override fun processImageProxy(image: ImageProxy, graphicOverlay: GraphicOverlay) {
        ...
        if (isMlImageEnabled(graphicOverlay.getContext())) {
            val mlImage =
                MediaMlImageBuilder(image.getImage()!!)
            ...
            requestDetectInImage(
                ...
                .addOnCompleteListener(OnCompleteListener { results: Task<T?>? -> image.close() })
            return
        }
        ...
        return task.addOnSuccessListener(
            ...
            OnSuccessListener { results: T? ->
                ...
                if (frameProcessedInOneSecondInterval == 1) {
                    Timber.v("Num of Runs: $numRuns")
                    Timber.v("Frame latency: max=${maxFrameMs}, min=${minFrameMs}, avg=${totalFrameMs / numRuns}")
                    Timber.v("Detector latency: max=${maxDetectorMs}, min=${minDetectorMs}, avg=${totalDetectorMs / numRuns}")
                    val mi = ActivityManager.MemoryInfo()
                    activityManager.getMemoryInfo(mi)
                    val availableMegs = mi.availMem / 0x100000L
                    Timber.v("Memory available in system: $availableMegs MB")
                ...
                this@VisionProcessorBase.onSuccess(results as T, graphicOverlay)
                if (!shouldHideDetectionInfo(graphicOverlay.getContext())) {
                    graphicOverlay.add(
            ...
            .addOnFailureListener(
                ...
                OnFailureListener { e: Exception? ->
                    graphicOverlay.clear()
                    graphicOverlay.postInvalidate()
                    val error = "Failed to process. Error: " + e!!.getLocalizedMessage()
                    Toast.makeText(
